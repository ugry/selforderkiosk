#!/usr/bin/env bash
# ============================================================
# Restaurant Kiosk – Order Monitor Machine Uninstaller
# Removes: .venv, logs/, install log files, ~/.xinitrc
# Does NOT touch system packages or the source files.
# Run as: sudo bash uninstall_monitor.sh
#         sudo bash uninstall_monitor.sh --force
# ============================================================
set -euo pipefail
...
[[ $EUID -ne 0 ]] && { echo -e "${RED}Please run as root: sudo bash uninstall_monitor.sh${NC}"; exit 1; }
...
echo -e "${RED}${BOLD}"
echo "╔══════════════════════════════════════════════════════╗"
echo "║      ORDER MONITOR MACHINE – UNINSTALL / CLEANUP    ║"
echo "╚══════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
echo ""
echo -e "${GREEN}${BOLD}✅  Order Monitor cleanup complete.${NC}"
echo ""
