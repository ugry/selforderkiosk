#!/usr/bin/env bash
# ============================================================
# Restaurant Kiosk – Kitchen Display Machine Uninstaller
# Removes: .venv, logs/, install log files, ~/.xinitrc
# Does NOT touch system packages or the source files.
# Run as: sudo bash uninstall_kitchen.sh
#         sudo bash uninstall_kitchen.sh --force
# ============================================================
set -euo pipefail
...
[[ $EUID -ne 0 ]] && { echo -e "${RED}Please run as root: sudo bash uninstall_kitchen.sh${NC}"; exit 1; }
...
echo -e "${RED}${BOLD}"
echo "╔══════════════════════════════════════════════════════╗"
echo "║    KITCHEN DISPLAY MACHINE – UNINSTALL / CLEANUP    ║"
echo "╚══════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
echo ""
echo -e "${GREEN}${BOLD}✅  Kitchen Display cleanup complete.${NC}"
echo ""
